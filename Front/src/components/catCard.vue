      ...
      <div class="card h-100">
        ...
        <div class="card-der">
          <div v-if="paid && paid.length">
            <h4 class="cart">
              <button class="btn btn-success"><i class="icon-cart" @click="addToCart()"></i></button>
            </h4>
          </div>
          <div v-if="free && free.length">
            <h4 class="cart">
              <button class="btn btn-success"><router-link :to="{name: 'market', params: {user: this.$store.state.user, id: product._id}}"><i class="icon-download" @click="download(product)"></i></router-link></button>
            </h4>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    download(product) {
      ...
      axios.post("http://localhost:3000/market", 
      {
